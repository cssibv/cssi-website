...
try {
    ...
    switch ($action) {
        ...
        // ════════════════════════════════════
        // ANAF CUI LOOKUP (proxy către registrul ANAF)
        // ════════════════════════════════════
        case 'lookupCUI':
            $cuiRaw = isset($_GET['cui']) ? $_GET['cui'] : (isset($data['cui']) ? $data['cui'] : '');
            $cui = preg_replace('/[^0-9]/', '', $cuiRaw);
            if (!$cui || strlen($cui) < 2 || strlen($cui) > 10) {
                jsonResponse(['success' => false, 'error' => 'CUI invalid (aşteptăm 2-10 cifre)'], 400);
                break;
            }

            $postData = json_encode([['cui' => intval($cui), 'data' => date('Y-m-d')]]);
            $ch = curl_init('https://webservicesp.anaf.ro/PlatitorTvaRest/api/v9/ws/tva');
            ...
            if (!$result || !isset($result['found'])) {
                jsonResponse(['success' => false, 'error' => 'Răspuns invalid de la ANAF'], 502);
                break;
            }
            ...
            // Date structurate v9 (adresa_sediu_social are câmpuri precum sdenumire_Localitate, sdenumire_Judet, sdenumire_Strada, snumar_Strada)
            ...
            if (!empty($src)) {
                ...
                $strada = !empty($src['sdenumire_Strada']) ? trim($src['sdenumire_Strada']) : '';
                $nr = !empty($src['snumar_Strada']) ? trim($src['snumar_Strada']) : '';
                if ($strada) {
                    $stradaEtc = $strada . ($nr ? ' nr. ' . $nr : '');
                }
            }

            // Fallback: parse text brut (ex: "MUN. BRASOV, STR. FOO, NR. 10, JUD. BRASOV")
            if (!$oras && $fullAddr) {
            ...
            jsonResponse(['success' => true, 'data' => [
                ...
                'oras' => $oras,
                'judet' => $judet,
                'cod_postal' => isset($dg['cod_postal']) ? $dg['cod_postal'] : '',
